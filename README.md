# Minsu Kang

데이터 분석과 모델링을 실제 서비스 흐름까지 연결하는 일을 해왔습니다. 최근에는 LLM 평가 자동화, 한국어 RAG, 교통·해양 안전 도메인의 예측 모델을 중심으로 작업하고 있습니다.

[Email](mailto:daro980722@gmail.com) · [DACON](https://dacon.io/myprofile/457768/home)

## Focus

- LLM/RAG 시스템: 평가 자동화, 문서 수집·파싱, 검색 흐름 설계, 답변 품질 검증
- ML 서비스화: 정형·시계열 모델링, 설명가능성 분석, API와 대시보드 연결
- 도메인 문제 해결: 교통 안전, 해양 안전, 법률 NLP, 공공 데이터 기반 분석
- 경진대회 솔루션 정리: 재현 가능한 notebook과 공개용 repository 정리

## Selected Work

| Project | Description | Area |
| --- | --- | --- |
| [LLM Evaluation System](https://github.com/Minsu5452/LLM_Evaluation_System) | 해커톤 제출물을 여러 LLM으로 평가하고 결과를 callback으로 반환하는 평가 서버 | LLM Evaluation |
| [Driver Risk Web Platform](https://github.com/Minsu5452/Driver_Risk_Web_Platform) | 운수종사자 검사 데이터 업로드, 사고 위험 예측, 관리자 재학습, Windows 배포까지 포함한 웹 플랫폼 | ML Service |
| [Marine Domain RAG](https://github.com/Minsu5452/Marine_Domain_RAG) | 공개 법령 API를 수집·파싱하고 해양 법령 질의응답 흐름으로 구성한 RAG 데모 | RAG |
| [Marine Accident Risk Prediction](https://github.com/Minsu5452/Marine_Accident_Risk_Prediction) | 공개 사고·격자·기상 데이터를 결합한 격자×시간 단위 해양사고 위험 예측 데모 | Risk Modeling |
| [Driver Cognitive Risk Prediction](https://github.com/Minsu5452/Driver_Cognitive_Risk_Prediction) | 운수종사자 인지검사 시퀀스와 과거 이력을 활용한 교통사고 위험 예측 경진대회 솔루션 | Competition |

## Highlights

| Category | Result |
| --- | --- |
| Genomic Data Breed Classification | 1st / 716 teams |
| Court Judgment Prediction | 2nd / 506 teams |
| HD Hyundai AI Challenge | Preliminary 2nd, Final 6th |
| LG Online Sales Forecasting Hackathon | Preliminary Top 1.6%, Finalist |
| Kookmin AI Big Data Analytics Competition | Top 5.2% |
| Driver Cognitive Risk Prediction | Top 7.8% |
| Power Consumption Forecasting | Top 8.7% |
| Citrus Yield Prediction | Top 6.6% |
| AAiCON 2023 | SNP 품종 분류 연구 논문 발표 |

## Core Stack

| Area | Stack |
| --- | --- |
| Data / ML | Python, SQL, feature engineering, model evaluation, explainability |
| LLM / RAG | LLM evaluation, RAG, GraphRAG, embedding search, prompt workflow |
| Serving / Infra | FastAPI, Spring Boot, Docker, Redis, Nginx |
| Frontend / Data App | React, Vite, Mantine, Streamlit |

## Experience

| Period | Organization | Role |
| --- | --- | --- |
| 2025.12 - Present | DACON, Data Science Team | Manager |
| 2025.06 - 2025.11 | SureSoftTech, AX Applied Technology Team | AI Developer Intern |

## Repository Index

<details>
<summary>LLM / RAG / Service</summary>

- [LLM_Evaluation_System](https://github.com/Minsu5452/LLM_Evaluation_System): LLM 평가·채점 서버
- [Marine_Domain_RAG](https://github.com/Minsu5452/Marine_Domain_RAG): 해양 법령 RAG 파이프라인
- [Driver_Risk_Web_Platform](https://github.com/Minsu5452/Driver_Risk_Web_Platform): 교통안전 위험 예측 웹 플랫폼
- [Marine_Accident_Risk_Prediction](https://github.com/Minsu5452/Marine_Accident_Risk_Prediction): 해양사고 위험 예측 데모

</details>

<details>
<summary>Competition Solutions</summary>

- [Genomic_Data_Breed_Classification](https://github.com/Minsu5452/Genomic_Data_Breed_Classification): 유전체 정보 품종 분류 1위 솔루션
- [Court_Judgment_Prediction](https://github.com/Minsu5452/Court_Judgment_Prediction): 법원 판결 예측 2위 솔루션
- [Ship_Waiting_Time_Prediction](https://github.com/Minsu5452/Ship_Waiting_Time_Prediction): HD현대 AI Challenge 솔루션 아카이브
- [LG_Online_Sales_Forecasting](https://github.com/Minsu5452/LG_Online_Sales_Forecasting): 온라인 채널 제품 판매량 예측
- [Power_Consumption_Forecasting](https://github.com/Minsu5452/Power_Consumption_Forecasting): 전력사용량 예측
- [Citrus_Yield_Prediction](https://github.com/Minsu5452/Citrus_Yield_Prediction): 감귤 착과량 예측
- [Driver_Cognitive_Risk_Prediction](https://github.com/Minsu5452/Driver_Cognitive_Risk_Prediction): 운수종사자 인지 특성 위험 예측
- [Industrial_Lead_Lag_Forecasting](https://github.com/Minsu5452/Industrial_Lead_Lag_Forecasting): 산업 지표 lead-lag 예측
- [Smart_Farm_Yield_Energy_Prediction](https://github.com/Minsu5452/Smart_Farm_Yield_Energy_Prediction): 스마트팜 수확량·에너지 예측
- [Traffic_Accident_Prediction](https://github.com/Minsu5452/Traffic_Accident_Prediction): 날씨 기반 교통사고 분석
- [Lotte_Members_Customer_Analytics](https://github.com/Minsu5452/Lotte_Members_Customer_Analytics): L.POINT 고객 데이터 분석

</details>

<details>
<summary>Academic / Course / Activity</summary>

- [AI_Frenz_2023_Publication](https://github.com/Minsu5452/AI_Frenz_2023_Publication): SNP 품종 분류 학술대회 기록
- [ML_Team_Project](https://github.com/Minsu5452/ML_Team_Project): 머신러닝 수업 과제와 Kaggle 경진대회
- [DL_Team_Project](https://github.com/Minsu5452/DL_Team_Project): 이미지 컬러화 fine-tuning 프로젝트
- [Restaurant_Topic_Modeling](https://github.com/Minsu5452/Restaurant_Topic_Modeling): 국민대·정릉시장 맛집 리뷰 토픽 모델링
- [Supporting_Marginalized_Communities](https://github.com/Minsu5452/Supporting_Marginalized_Communities): 청각장애인 지원 AI 아이디어 프로토타입
- [Kookmin_Mentoring_14th](https://github.com/Minsu5452/Kookmin_Mentoring_14th): 국민대 연결고리 멘토링 활동 기록

</details>

## Education & Certificates

- B.S. in AI Big Data Convergence Management, Kookmin University
- SQLD, Korea Data Agency
- Big Data Analysis Engineer written exam, Korea Data Agency
- TOEIC Speaking Intermediate Mid 3
