...
| 영역 | 다루는 범위 | 관련 작업 |
| --- | --- | --- |
| 분석·모델링 | 정형·시계열 데이터 전처리, 피처 설계, 검증 전략, 결과 해석 | 교통 위험 예측, 전력·판매량·선박 대기시간 예측 |
| NLP·RAG | 법률 NLP, 한국어 문서 파싱, 검색 기반 질의응답, 평가 데이터 구성 | 법원 판결 예측, 해양 법령 도메인 RAG |
| 서비스·API | FastAPI AI 엔진, queue/worker, callback, Docker 기반 실행 환경 | LLM 평가 시스템, 해양사고 예측 API, 운수종사자 위험 예측 플랫폼 |
| 웹·대시보드 | React 화면, Spring Boot 백엔드, 관리자 기능, 격자·진단 대시보드 | 운수종사자 웹 플랫폼, 해양사고 위험 대시보드 |
